...
\textbf{浙江大学} \hfill 2025.9 -- 至今\\[0.3em] % 标题和正文间加一点距离
工业设计\quad 大一 
\begin{itemize}[nosep]
...
\begin{itemize}[nosep, topsep=\intraProjectListTopSep]
    \item \textbf{技术栈}：Next.js 16, React 10, TypeScript, Tailwind CSS, Supabase,Vercel
\end{itemize}
...
项目描述：来自UCB CS106L的project,给定两个维基词条，在维基百科链接构成的词网中自动求解从起点到终点的最短路径。采用离线建图：先解析维基数据构建邻接表并持久化到本地，查询时在内存中运行 BFS 得到最短路径与步数。
\begin{itemize}[nosep, topsep=\intraProjectListTopSep]
    \item \textbf{技术栈}：C++, QtCreator, CMake, Wikipedia REST API，邻接表 + BFS
\end{itemize}
...
项目描述：基于Arduino平台，设计并实现了一款四路红外小车循迹系统。支持PID巡线、直角弯识别与终点黑框检测，实现自动寻迹与停车。
\begin{itemize}[nosep, topsep=\intraProjectListTopSep]
